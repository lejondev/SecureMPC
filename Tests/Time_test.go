...
func TestTimeKeyGen(t *testing.T) {
	f, err := os.Create("PATH GOES HERE")
	defer f.Close()
	check(err)
...
func TestTimeAll(t *testing.T) {
	f1, err := os.Create("PATH GOES HERE")
	f2, err := os.Create("PATH GOES HERE")
	f3, err := os.Create("PATH GOES HERE")
	defer f1.Close()
	defer f2.Close()
	defer f3.Close()
	...
	for i := 0; i < 4; i++ {
		...
		for j := 1; j <= 4; j++ {
			...
			fmt.Println("Time passed: " + time.Since(startTime1).String() + " " + info)
			...
			fmt.Println("Time passed: " + time.Since(startTime3).String() + " " + info)
